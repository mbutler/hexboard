class SettlementGenerator {
    constructor() {
      ...
      this.humanTypesRules = [
        { type: 'Bandits', range: [1, 25] },
        { type: 'Brigands', range: [26, 85] },
        { type: 'Berserkers', range: [86, 97] },
        { type: 'Dervishes', range: [98, 100] }
      ]
      this.characterTypesRules = [
        { type: 'Cleric', range: [1, 18], level: '9th-12th' },
        { type: 'Druid', range: [19, 20], level: '12th-13th' },
        { type: 'Fighter', range: [21, 65], level: '9th-12th' },
        { type: 'Paladin', range: [66, 66], level: '9th-10th' },
        { type: 'Ranger', range: [67, 68], level: '10th-13th' },
        { type: 'Magic-User', range: [69, 80], level: '11th-14th' },
        { type: 'Illusionist', range: [81, 85], level: '10th-13th' },
        { type: 'Thief', range: [86, 93], level: '10th-14th' },
        { type: 'Assassin', range: [94, 96], level: '14th' },
        { type: 'Monk', range: [97, 99], level: '9th-12th' },
        { type: 'Bard', range: [100, 100], level: '23rd' }
      ]
    }
    ...
    getCastleDetails() {
      ...
      if (inhabitants === 'Humans') {
        inhabitants = this.getHumanType()
      } else if (inhabitants === 'Character-types') {
        inhabitants = this.getCharacterType()
      }
    ...
    getHumanType() {
      ...
      for (const rule of this.humanTypesRules) {
        if (humanRoll >= rule.range[0] && humanRoll <= rule.range[1]) {
          return rule.type
        }
      }
      return 'Unknown'
    ...
    getCharacterType() {
      const characterRoll = Math.floor(Math.random() * 100) + 1
      for (const rule of this.characterTypesRules) {
        if (characterRoll >= rule.range[0] && characterRoll <= rule.range[1]) {
          return `${rule.type} (${rule.level})`
        }
      }
      return 'Unknown'
    }
